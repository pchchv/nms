...
const { nmsrc, api, apis } = require(process.cwd() + '/test/utils/fixtures')
...
describe('Client configuration', () => {
    ...
    describe('.sandbox', () => {
        beforeEach(() => {
            payload = JSON.parse( JSON.stringify(nmsrc) )
        })
        ...
        describe('.addresses', () => {
            ['from', 'replyTo'].forEach(property => {
                it(`error - ${property}.name is required`, (done) => {
                    payload.consumer.addresses = { [property] : { name: null, email : 'hello@john.galt.com' } }
                    const error = configurationSchema.validate(payload, { abortEarly: true, allowUnknown: false })?.error
    ...
    describe('.mode', () => {
        ...
        describe('.api', () => {
            beforeEach(() => {
                payload = JSON.parse( JSON.stringify(mnsrc) )
